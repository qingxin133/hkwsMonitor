...
public interface HCNetSDK extends StdCallLibrary {
	...
	public static class NET_DVR_PDC_RULE_CFG_V41 extends Structure
    {
              public int                          dwSize;
              public byte                           byEnable;
              public byte[] byRes1 = new byte[23];
	...
	public static class NET_DVR_PDC_RULE_CFG_V42 extends Structure {
	    public int  dwSize; //结构大小

	    public byte   byEnable; //是否激活规则;

	    public byte   byOSDEnable;//客流统计OSD显示是否启用, 0-否（0-无），1-是（0-进入和离开）， 2-进入， 3-离开  对于老设备的不启用OSD叠加对应（无），启用对应（进入/离开），4-经过人数，5-进入和经过。

	    public byte   byCurDetectType;//当前检测区域类型，0-多边形，1-检测线, 2-折线检测

	    public byte   byInterferenceSuppression; //干扰抑制，按位表示,0-未勾选,1-勾选，bit0-阴影，bit1-徘徊，bit2-推车

	    public NET_VCA_POINT   struOSDPoint;//客流统计显示OSD显示左上角坐标
	    ...
	    public byte[] byRes2 = new byte[6];// 保留字节 
	    ...
	    public byte    byStreamOverlayRuleInfos;//码流智能信息叠加 0-否，1-是

	    public byte   byRes3;// 保留字节

	    public NET_VCA_LINE  struLine;//检测线

	    public byte byHeightFilterEnable;//高度过滤是否开启，0-保留，1-关闭，2-开启

	    public byte  byDetectThreshold;//检测阈值,范围[0,100],默认 90(内部使用，暂不开放)用于设定算法库中被检测目标的置信度阈值，置信度低于该阈值的目标，不用于跟踪计数；

	    public byte  byAidedTrackEnabled;//辅助跟踪开关，0~开启、1~关闭，默认开启(内部使用) 用于开启和关闭算法跟踪模块中的光流辅助跟踪，跟踪客流人员，增加精准度。

	    public byte byRes4;

	    public float fHeightFilter;//过滤高度，单位：厘米，默认值：120厘米，范围：40-200厘米。byHeightFilterEnable为2是才有效

	    public byte byCalibrateType;//标定类型，0-未进行标定，1-自动标定，2-手动标定

	    public byte byCountingType;//计数触发方式 0-无,1-报警输入触发,2-视频分析触发

	    public byte bySignalType;//信号类型 0-电平，1-脉冲

	    public byte byRS485TransmissionEnabled;//启用485传输 0-关闭，1-开启

	    public float fTiltAngle;//俯仰角,单位：度；俯仰角默认值：0；俯仰角范围：0-180度,只读

	    public float fHeelAngle;//倾斜角,单位：度；倾斜角默认值：0；倾斜角范围：-90-90度,只读

	    public float fHeight;//高度，高度单位：厘米；高度默认值300厘米：高度范围：200-500厘米,当byCalibrateType为2时设置有效，其余时只读

	    public NET_VCA_POLYGON struCountPolygon;//计数区域：该参数默认为只读属性，若智能通道分析能力中返回了<CountingPolygon>能力节点，则代表该参数支持配置。

	    public NET_VCA_POLYGON struAutoCalibPolygon;//标定区域，当byCalibrateType为1时有效

	    public NET_DVR_TIME_EX struDailyResetTime; //定时清零

	    public NET_VCA_POLYLINE struPolyLine;//折线检测区域（配置折线后，struLine不做显示）
	  ...
	  *
	  */
	public static class NET_DVR_PDC_RULE_COND extends Structure {

	    public int  dwSize; //结构大小

	    public int  dwChannel; //通道号

	    public int dwID; //场景ID，兼容球机多场景概念，兼容老版本SDK配置新设备时，保留字节为0的情况，所以这个字节为0时，也默认为场景1
	...
	//折线参数结构体。
	 public static class NET_VCA_POLYLINE  extends Structure
	 {
	 	 public NET_VCA_POINT[] struPoint = new NET_VCA_POINT[4];  
	 }
	//线结构参数
	 ...
	 public static class NET_DVR_PDC_ENTER_DIRECTION extends Structure
	 {
    ...
    int NET_DVR_GetNextRemoteConfig (NativeLong lHandle, Pointer lpOutBuff, int dwOutBuffSize);
  ...
  //--------------------------------------------------------------
    //客流量数据查询条件结构
   public static class NET_DVR_PDC_QUERY_COND extends Structure
   {
   	public int             dwSize;
   	public int             dwChannel;
   	...
   	/**
   	 * 查询类型：0- 无效值，1- 日报表，2- 周报表，3- 月报表，4- 年报表 
   	 */
   	public byte              byReportType;
   	public byte              byEnableProgramStatistics;
   	public byte              byTriggerPeopleCountingData;
   	public byte              byRes1;
   ...
   //客流量数据查询结果结构
   public static class NET_DVR_PDC_RESULT extends Structure
   {
   	 public int                   dwSize;
   	 public NET_DVR_TIME_EX       struStartTime = new NET_DVR_TIME_EX();
   	 public NET_DVR_TIME_EX       struEndTime = new NET_DVR_TIME_EX();
   	 ...
   	 public NET_DVR_PROGRAM_INFO  struProgramInfo = new NET_DVR_PROGRAM_INFO();
   ...
   public static interface FRemoteConfigCallback extends StdCallCallback {
    ...
    /***宏定义***/
    //常量
   public static final int NET_DVR_SET_PDC_RULECFG_V42 = 3406;//设置客流量统计规则
   public static final int MAX_ALERTLINE_NUM = 8;
   public static final int NET_DVR_GET_PDC_RULECFG_V42 = 3405;	//DVR本地登陆名 
   public static final int NET_DVR_GET_PDC_RESULT = 5089;	//DVR本地登陆名
...
//校时结构参数
    public static class NET_DVR_TIME extends Structure {//校时结构参数
      ...
      public String toString() {
            return "NET_DVR_TIME.dwYear:" + dwYear  + "NET_DVR_TIME.dwMonth: " + dwMonth  + "NET_DVR_TIME.dwDay: " + dwDay + "NET_DVR_TIME.dwHour:" + dwHour  + "NET_DVR_TIME.dwMinute: " + dwMinute  + "NET_DVR_TIME.dwSecond:" + dwSecond;
        }
